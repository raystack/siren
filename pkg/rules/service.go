...
func (service Service) Get(name, namespace, groupName string, template string) ([]domain.Rule, error) {
	rules, err := service.repository.Get(name, namespace, groupName, template)
	if err != nil {
		return nil, err
	}
